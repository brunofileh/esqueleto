...
class PrintView extends Pdf
{
	...
	public function __construct(array $params=[])
	{
		...
		$topo = $this->view->render('@app/views/pdf/view/header', [
			'orientation' => $this->orientation,
			'titulo' => $params['titulo'],
			'subtitulo' => $params['subtitulo'],
			'subtitulo2' => isset($params['subtitulo2']) ? $params['subtitulo2'] : '',
			'ano' => $params['ano'],
		]);
		...
		if (!is_dir(Yii::getAlias("@runtime/tmp/"))) {
			mkdir(Yii::getAlias("@runtime/tmp/"), 0775);
		}
	}
}
